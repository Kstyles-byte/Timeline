      ...
      <AnimatePresence mode="wait">
        {isEditing ? (
          ...
          >
            {editItems.map((item, index) => (
              <div key={index} className="flex items-center gap-3">
                ...
                <input
                  ...
                  className="flex-1 p-3 border-2 border-slate-600 rounded-xl focus:border-teal-500 focus:outline-none bg-slate-700 text-gray-100 placeholder-gray-400"
            ...
            <motion.button
              onClick={addItem}
              className="flex items-center gap-2 px-4 py-3 border-2 border-dashed border-slate-600 rounded-xl hover:border-teal-500 hover:bg-teal-900/20 transition-colors w-full justify-center"
              whileHover={{ scale: 1.02 }}
              whileTap={{ scale: 0.98 }}
            >
              <Plus size={20} className="text-teal-400" />
              <span className="text-teal-400 font-medium">Add Non-Negotiable</span>
            </motion.button>
          </motion.div>
        ) : (
          <motion.div
          ...
          >
            {nonNegotiables.map((item, index) => (
              <motion.div
                ...
                className="bg-slate-700 p-4 rounded-xl border-l-4 border-teal-400 flex items-center gap-4"
              >
                <CheckCircle className="text-teal-400 flex-shrink-0" size={20} />
